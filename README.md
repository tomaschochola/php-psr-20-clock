...
This library creates an abstraction over system time, implementing the PSR-20 Clock interface to ensure testability and temporal determinism. It provides both a system-aligned `NowClock` for production and a strictly controllable `FixedClock` for testing environments.

By removing direct reliance on global time functions, this package eliminates flaky tests related to time-sensitive logic. It allows the entire architecture to reason about time as an explicit dependency rather than a global mutable state.
